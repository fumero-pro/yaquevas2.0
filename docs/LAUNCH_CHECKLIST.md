...
- [~] Proveedor de email transaccional: **conectado de verdad** (Resend vía API REST, ver `docs/EMAIL_SETUP.md` — mismo aviso que Stripe, código listo pero no probado contra cuenta real, falta que el usuario cree la suya). Ya envía: bienvenida + confirmación de email, recuperación de contraseña, y TODAS las notificaciones internas (aceptación, pago, entrega, recompensa de referido) si `notif_prefs.email` está activo — centralizado en `backend/src/lib/notify.js`. Sin la clave, todo sigue simulado en logs, nunca rompe nada.
- [~] Proveedor SMS: **conectado de verdad** (Twilio vía API REST, ver `docs/SMS_SETUP.md`, mismo patrón simulado/real). Verificación de teléfono por código de 6 dígitos (`POST /api/me/phone/send-code` / `/verify-code`). Falta que el usuario cree su propia cuenta de Twilio y confirme que llegan SMS reales.
