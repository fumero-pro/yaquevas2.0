...
function insertDeliveredBooking(db, { id, deliveredAt }) {
...
function insertReview(db, { bookingId, reviewerId, revieweeId, rating }) {
  db.prepare(
...
test('una reseña sola (sin la del otro lado) no cuenta todavía si la entrega es reciente', () => {
  const db = createTestDb();
  insertUser(db, 'usr_sender'); insertUser(db, 'usr_traveler');
  insertDeliveredBooking(db, { id: 'book_1', deliveredAt: new Date().toISOString() });
  insertReview(db, { bookingId: 'book_1', reviewerId: 'usr_sender', revieweeId: 'usr_traveler', rating: 2 });

  const summary = getVisibleRatingSummary(db, 'usr_traveler');
  assert.equal(summary.rating_count, 0);
  assert.equal(summary.rating_avg, null);
});

test('cuando ambas partes han reseñado, las dos reseñas se vuelven visibles', () => {
  const db = createTestDb();
  insertUser(db, 'usr_sender'); insertUser(db, 'usr_traveler');
  insertDeliveredBooking(db, { id: 'book_2', deliveredAt: new Date().toISOString() });
  insertReview(db, { bookingId: 'book_2', reviewerId: 'usr_sender', revieweeId: 'usr_traveler', rating: 4 });
  insertReview(db, { bookingId: 'book_2', reviewerId: 'usr_traveler', revieweeId: 'usr_sender', rating: 5 });

  const travelerSummary = getVisibleRatingSummary(db, 'usr_traveler');
  const senderSummary = getVisibleRatingSummary(db, 'usr_sender');
...
test('una reseña sola se vuelve visible pasados 14 días desde la entrega, aunque el otro lado nunca reseñe', () => {
  const db = createTestDb();
  insertUser(db, 'usr_sender'); insertUser(db, 'usr_traveler');
  const fifteenDaysAgo = new Date(Date.now() - 15 * 24 * 60 * 60 * 1000).toISOString();
  insertDeliveredBooking(db, { id: 'book_3', deliveredAt: fifteenDaysAgo });
  insertReview(db, { bookingId: 'book_3', reviewerId: 'usr_sender', revieweeId: 'usr_traveler', rating: 3 });
...
test('exactamente en el límite de 13 días todavía no es visible sin la reseña del otro lado', () => {
  const db = createTestDb();
  insertUser(db, 'usr_sender'); insertUser(db, 'usr_traveler');
  const thirteenDaysAgo = new Date(Date.now() - 13 * 24 * 60 * 60 * 1000).toISOString();
  insertDeliveredBooking(db, { id: 'book_4', deliveredAt: thirteenDaysAgo });
  insertReview(db, { bookingId: 'book_4', reviewerId: 'usr_sender', revieweeId: 'usr_traveler', rating: 1 });
