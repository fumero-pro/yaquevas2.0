...
test('itemsToUsage suma bultos por tipo y estima kg', async () => {
...
test('itemsToUsage ignora tipos de bulto desconocidos sin lanzar', async () => {
  const usage = itemsToUsage([{ item_type: 'inventado', quantity: 5 }]);
  assert.deepEqual(usage, emptyUsage());
});

test('fitsInTrip: cabe cuando hay espacio y peso suficiente', async () => {
...
test('fitsInTrip: no cabe si excede el peso aunque el volumen quepa', async () => {
...
test('capacityStatus calcula porcentajes y sugerencias sin capacidad configurada', async () => {
...
test('itemsToUsage: una tabla de surf (XXL) se contabiliza como objeto_voluminoso', async () => {
...
test('fitsInTrip: una tabla de surf (XXL) cabe si el viaje declara espacio para objetos voluminosos', async () => {
...
test('fitsInTrip: un bulto extra grande (XXXL) no cabe si el viaje no declaró espacio para ese tipo', async () => {
...
test('addUsage no muta los objetos originales', async () => {
  const a = emptyUsage();
