...
function register(router, db) {
  ...
  router.get('/api/matching/for-trip/:id', async (req, res, body, params) => {
    ...
    res.json({
      matches: matches.map((m) => ({ shipment: serializeShipment(m.shipment, db.prepare('SELECT * FROM shipment_items WHERE shipment_id = ?').all(m.shipment.id)), compatibilidad_pct: m.score })),
    });
  });
