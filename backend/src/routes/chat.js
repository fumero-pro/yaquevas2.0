...
function register(router, db) {
  router.get('/api/bookings/:id/messages', async (req, res, body, params) => {
    ...
    const rows = await db.prepare('SELECT * FROM messages WHERE booking_id = ? ORDER BY created_at ASC').all(booking.id);
    await db.prepare("UPDATE messages SET read = 1 WHERE booking_id = ? AND sender_id != ? AND read = 0")
  ...
  router.post('/api/bookings/:id/messages', async (req, res, body, params) => {
    ...
    await notify(db, otherUserId, 'mensaje', 'Nuevo mensaje', text || 'Te ha enviado una acción rápida.', booking.id);

    const saved = await db.prepare('SELECT * FROM messages WHERE id = ?').get(id);
