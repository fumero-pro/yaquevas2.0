...
async function auditLog(db, adminId, action, target, oldValue, newValue) {
  await db.prepare(
...
function register(router, db) {
  ...
  router.get('/api/admin/dashboard', async (req, res) => {
    ...
    const count = async (sql, ...args) => (await db.prepare(sql).get(...args))?.c || 0;
    const sum = async (sql, ...args) => (await db.prepare(sql).get(...args))?.s || 0;

    const stats = {
      usuarios_totales: await count('SELECT COUNT(*) c FROM users'),
      viajeros_activos: await count("SELECT COUNT(DISTINCT user_id) c FROM trips WHERE status = 'publicado'"),
      envios_totales: await count('SELECT COUNT(*) c FROM shipments'),
      viajes_totales: await count('SELECT COUNT(*) c FROM trips'),
      operaciones_completadas: await count("SELECT COUNT(*) c FROM bookings WHERE status IN ('pago_liberado','finalizado')"),
      operaciones_en_curso: await count("SELECT COUNT(*) c FROM bookings WHERE status NOT IN ('pago_liberado','finalizado','cancelado','rechazado')"),
      cancelaciones: await count("SELECT COUNT(*) c FROM bookings WHERE status = 'cancelado'"),
      incidencias_abiertas: await count("SELECT COUNT(*) c FROM disputes WHERE status IN ('abierta','en_revision','esperando_informacion')"),
      dinero_procesado: await sum("SELECT COALESCE(SUM(amount),0) s FROM payments WHERE type = 'cobro_remitente' AND status = 'completado'"),
      comision_yaquevas_total: await sum("SELECT COALESCE(SUM(amount),0) s FROM payments WHERE type = 'comision_yaquevas' AND status = 'completado'"),
      payouts_viajeros_total: await sum("SELECT COALESCE(SUM(amount),0) s FROM payments WHERE type = 'payout_viajero' AND status = 'completado'"),
      pagos_pendientes: await count("SELECT COUNT(*) c FROM bookings WHERE status = 'pago_realizado'"),
      reembolsos_totales: await sum("SELECT COALESCE(SUM(amount),0) s FROM refunds WHERE status = 'completado'"),
    };

    const rutasPopulares = await db
      .prepare(
  ...
  router.get('/api/admin/config', async (req, res) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    res.json({ config: await getConfig(db) });
  });

  router.put('/api/admin/config', async (req, res, body) => {
    const admin = await requireRole(req, res, db, ['superadmin']);
    if (!admin) return;
    const before = await getConfig(db);
    for (const [key, value] of Object.entries(body)) {
      await setConfigValue(db, key, value, admin.id);
    }
    const after = await getConfig(db);
    await auditLog(db, admin.id, 'actualizar_configuracion', 'config', JSON.stringify(before), JSON.stringify(after));
  ...
  router.get('/api/admin/prohibited-items', async (req, res) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    res.json({ prohibited_items: await db.prepare('SELECT * FROM prohibited_items ORDER BY category, name').all() });
  });

  router.post('/api/admin/prohibited-items', async (req, res, body) => {
    const admin = await requireRole(req, res, db, ['superadmin', 'admin']);
    if (!admin) return;
    ...
    await auditLog(db, admin.id, 'crear_objeto_prohibido', id, null, body.name);
  ...
  router.put('/api/admin/prohibited-items/:id', async (req, res, body, params) => {
    const admin = await requireRole(req, res, db, ['superadmin', 'admin']);
    if (!admin) return;
    const before = await db.prepare('SELECT * FROM prohibited_items WHERE id = ?').get(params.id);
    if (!before) return res.status(404).json({ error: 'No encontrado.' });
    await db.prepare('UPDATE prohibited_items SET name = ?, category = ?, note = ?, active = ? WHERE id = ?').run(
    ...
    await auditLog(db, admin.id, 'editar_objeto_prohibido', params.id, JSON.stringify(before), JSON.stringify(body));
    res.json({ ok: true });
  });
  ...
  router.get('/api/admin/operations', async (req, res, body, params, query) => {
    ...
    res.json({ operations: await db.prepare(sql).all(...args) });
  });
  ...
  router.get('/api/admin/disputes', async (req, res, body, params, query) => {
    ...
    if (query.status) { sql += ' AND status = ?'; args.push(query.status); }
    sql += ' ORDER BY created_at DESC LIMIT 200';
    res.json({ disputes: await db.prepare(sql).all(...args) });
  });

  router.put('/api/admin/disputes/:id', async (req, res, body, params) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    const before = await db.prepare('SELECT * FROM disputes WHERE id = ?').get(params.id);
    if (!before) return res.status(404).json({ error: 'Incidencia no encontrada.' });
    const now = new Date().toISOString();
    await db.prepare('UPDATE disputes SET status = ?, admin_id = ?, updated_at = ? WHERE id = ?').run(
      body.status || before.status, admin.id, now, params.id
    );
    await auditLog(db, admin.id, 'actualizar_incidencia', params.id, before.status, body.status);
    res.json({ ok: true });
  });
  ...
  router.get('/api/admin/users', async (req, res, body, params, query) => {
    ...
    sql += ' ORDER BY created_at DESC LIMIT 200';
    res.json({ users: await db.prepare(sql).all(...args) });
  });
  ...
  router.put('/api/admin/users/:id/role', async (req, res, body, params) => {
    ...
    const before = await db.prepare('SELECT role FROM users WHERE id = ?').get(params.id);
    if (!before) return res.status(404).json({ error: 'Usuario no encontrado.' });
    await db.prepare('UPDATE users SET role = ? WHERE id = ?').run(body.role, params.id);
    await auditLog(db, admin.id, 'cambiar_rol_usuario', params.id, before.role, body.role);
    res.json({ ok: true });
  });
  ...
  router.put('/api/admin/users/:id/active', async (req, res, body, params) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    const before = await db.prepare('SELECT active FROM users WHERE id = ?').get(params.id);
    if (!before) return res.status(404).json({ error: 'Usuario no encontrado.' });
    await db.prepare('UPDATE users SET active = ? WHERE id = ?').run(body.active ? 1 : 0, params.id);
    await auditLog(db, admin.id, 'activar_desactivar_usuario', params.id, before.active, body.active ? 1 : 0);
    res.json({ ok: true });
  });
  ...
  router.get('/api/admin/audit-log', async (req, res) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    res.json({ audit_log: await db.prepare('SELECT * FROM audit_log ORDER BY created_at DESC LIMIT 300').all() });
  });
  ...
  router.post('/api/admin/legal-documents', async (req, res, body) => {
    ...
    await auditLog(db, admin.id, 'publicar_documento_legal', doc_type, null, version);
    res.status(201).json({ id });
  });
  ...
  router.get('/api/admin/legal-documents/:doc_type/history', async (req, res, body, params) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    res.json({ history: await db.prepare('SELECT id, doc_type, version, created_by, created_at FROM legal_documents WHERE doc_type = ? ORDER BY created_at DESC').all(params.doc_type) });
  });
  ...
  router.post('/api/admin/pricing-samples', async (req, res, body) => {
    ...
    await auditLog(db, admin.id, 'anadir_muestra_precio', id, null, `${origin_island}-${destination_island}: ${price}€ (${source})`);
    res.status(201).json({ id });
  });
  ...
  router.get('/api/admin/pricing-samples', async (req, res) => {
    const admin = await requireAdmin(req, res, db);
    if (!admin) return;
    const rows = await db.prepare('SELECT * FROM pricing_reference_samples ORDER BY captured_at DESC LIMIT 200').all();
  ...
  router.delete('/api/admin/pricing-samples/:id', async (req, res, body, params) => {
    const admin = await requireRole(req, res, db, ['superadmin', 'admin']);
    if (!admin) return;
    const sample = await db.prepare('SELECT * FROM pricing_reference_samples WHERE id = ?').get(params.id);
    if (!sample) return res.status(404).json({ error: 'Muestra no encontrada.' });
    await db.prepare('DELETE FROM pricing_reference_samples WHERE id = ?').run(params.id);
    await auditLog(db, admin.id, 'borrar_muestra_precio', params.id, `${sample.route_key}: ${sample.price}€`, null);
    res.json({ ok: true });
  });
  ...
  router.get('/api/admin/export/:entity', async (req, res, body, params) => {
    ...
    const rows = await db.prepare(`SELECT * FROM ${table} ORDER BY created_at DESC LIMIT 5000`).all();
    if (rows.length === 0) {
