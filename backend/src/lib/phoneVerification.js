...
async function createPhoneCode(db, userId, phone) {
...
async function verifyPhoneCode(db, userId, phone, code) {
  ...
  if (hashCode(String(code || '')) !== row.code_hash) {
    await db.prepare('UPDATE phone_verifications SET attempts = attempts + 1 WHERE id = ?').run(row.id);
    return 'invalido';
  }
  await db.prepare('UPDATE phone_verifications SET used = 1 WHERE id = ?').run(row.id);
  return 'ok';
}
